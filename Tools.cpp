...
int Tools::CountUnitsOfType(BWAPI::UnitType type, const BWAPI::Unitset& units, bool complete,bool idle)
{
    int sum = 0;
    for (auto& unit : units)
    {
...
const BWEM::ChokePoint* Tools::getChokePoint(BWAPI::TilePosition a, BWAPI::TilePosition b)
{
    
    auto chokepoints = BWEM::Map::Instance().GetNearestArea(a)->ChokePointsByArea();
    auto path = BWEM::Map::Instance().GetPath((BWAPI::Position)a, (BWAPI::Position)b);
