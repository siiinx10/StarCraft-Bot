...
void StarterBot::onFrame()
{
    ...
    if (m_enemy_found && !m_rushed)
    {
        ...
        if (Tools::CountUnitsOfType(zergling, BWAPI::Broodwar->self()->getUnits(), true, true) >= 6)
        {
...
void StarterBot::checkBuildOrder()
{
    ...
    else if (m_hydraDenBuilt)
    {
        ...
        {
            if (BWAPI::Broodwar->getFrameCount() > m_timer)
            {
                m_lurkers = 2;
                m_timer = 0;
                m_hydralisk = 11;
...
void StarterBot::sendAllZergs()
{
    const BWAPI::UnitType zergling = BWAPI::UnitTypes::Zerg_Zergling;
    const BWAPI::Unitset& myUnits = BWAPI::Broodwar->self()->getUnits();
    for (auto& unit : myUnits)
    {
        if (unit->getType() == zergling)
        {
            //std::cout << m_target;
            //Tools::SmartRightClick(unit, m_target);
            //unit->rightClick(m_target);
...
void StarterBot::burrowLurkers()
{
    const BWAPI::Unitset& Units = BWAPI::Broodwar->self()->getUnits();
    BWAPI::UnitType lurker = BWAPI::UnitTypes::Zerg_Lurker;
    auto loc = Tools::getChokePoint(BWAPI::Broodwar->self()->getStartLocation(), (BWAPI::TilePosition)m_target_position);
    for (auto& unit : Units)
    {
        if (unit->getType() == lurker && unit->isIdle() && !unit->isBurrowed())
        {
            ...
            {
                unit->move((BWAPI::Position)loc->Center());
            }
        }
...
void StarterBot::trainLurker()
{
    const BWAPI::UnitType lurker = BWAPI::UnitTypes::Zerg_Lurker;
    const int lurkersOwned = Tools::CountUnitsOfType(lurker, BWAPI::Broodwar->self()->getUnits());

    if (lurkersOwned < m_lurkers)
    {
        bool training = trainAdditionalUnit(lurker);
    }
}

void StarterBot::trainMutalisks()
{
    const BWAPI::UnitType mutalisk = BWAPI::UnitTypes::Zerg_Mutalisk;
    const int mutalisksOwned = Tools::CountUnitsOfType(mutalisk, BWAPI::Broodwar->self()->getUnits());

    if (mutalisksOwned < m_mutaliskWanted)
    {
        bool training = trainAdditionalUnit(mutalisk);
    }
}
...
void StarterBot::buildSpire()
{
    const BWAPI::UnitType spire = BWAPI::UnitTypes::Zerg_Spire;
    const int spireOwned = Tools::CountUnitsOfType(spire, BWAPI::Broodwar->self()->getUnits());
    if (spireOwned < m_spiresWanted)
    {
        if (!Tools::CanAfford(spire)) { return; }
...
void StarterBot::researchLurker()
{
    ...
    if (!(den->isResearching()))
    {
        ...
        m_timer = lurkertech.researchTime() + BWAPI::Broodwar->getFrameCount();
    }
}
